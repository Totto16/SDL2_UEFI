...
#define SDL_NUM_WARNING_STATUS 8
#define SDL_NUM_ERROR_STATUS   35

static const char *const __SDL_EFI_STATUS_To_String_convert_table_warning[] = {
...
SDL_COMPILE_TIME_ASSERT(__SDL_EFI_STATUS_To_String_convert_table_warning, SDL_arraysize(__SDL_EFI_STATUS_To_String_convert_table_warning) == SDL_NUM_WARNING_STATUS);

static const char *const __SDL_EFI_STATUS_To_String_convert_table_error[] = {
...
SDL_COMPILE_TIME_ASSERT(__SDL_EFI_STATUS_To_String_convert_table_error, SDL_arraysize(__SDL_EFI_STATUS_To_String_convert_table_error) == SDL_NUM_ERROR_STATUS);

const char *SDL_EFI_STATUS_To_String(EFI_STATUS Status)
{

    const char *result = "<Unknown Status>";

    if (RETURN_ERROR(Status)) {
        //
        // Clear error bit
        //
        INTN Index = Status & ~MAX_BIT;
        if ((Index > 0) && (Index <= SDL_NUM_ERROR_STATUS)) {
            result = __SDL_EFI_STATUS_To_String_convert_table_error[Index - 1];
        }
    } else {
        INTN Index = Status;
        if (Index < SDL_NUM_WARNING_STATUS) {
            result = __SDL_EFI_STATUS_To_String_convert_table_warning[Index];
        }
    }
